...
def creat_max_df(month_index, year, district):
    ...
    final_max_df = pd.DataFrame(dist_year.iloc[month_index])
    final_max_df = final_max_df.reset_index()
    final_max_df.rename(columns = {'index' : "Day"}, inplace = True)
    final_max_df.rename(columns = {month_index : 'Tempareture'}, inplace = True)
    final_max_df.drop(final_max_df.head(3).index, inplace=True)
    final_max_df = final_max_df.reset_index(drop = True)
